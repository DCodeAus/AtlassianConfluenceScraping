...
**Stuck on a prompt?** Type `?` and hit Enter - every question these scripts ask answers back with what it actually wants, then asks again.
...
| Weird garbled characters in the Markdown, e.g. `Â` where a space or accent should be, or `â€™` instead of an apostrophe | An old bug in `confluence_extractor.ps1` - already fixed, new extractions come out clean. For content pulled before the fix, run `repair_garbled_text.ps1` (or `.py`) from the folder containing `confluence_export`/`confluence_markdown_export`. It'll fix what it can and tell you about anything it can't (usually means the original bytes are gone for good - re-extracting that one page is the real fix, `-Force` + typing `YOLO` will strip the leftover character instead if you'd rather not). Run it with no arguments and read what it prints - it explains itself. |
