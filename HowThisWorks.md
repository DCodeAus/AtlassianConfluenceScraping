...
- **The conversion steps never write back into `confluence_export/`**, only read it, so re-running any of them is safe. `repair_garbled_text` (step 2 above) is the one exception - modifying it in place is the whole point - but it always backs up first, so nothing's actually at risk.
